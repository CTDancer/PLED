...
pascal_labels = ['aeroplane', 'bicycle', 'bird', 'boat', 'bottle', 'bus', 'car', 'cat', 'chair',
...
for phase in ['train', 'val']:
        ...
        with open(os.path.join(args.load_path, 'VOCdevkit', 'VOC2012', 'ImageSets', 'Main', cat + '_' + phase + '.txt'), 'r') as f:
            for line in f:
                ...
                if int(label) == 1:
                    ...
                    if cat == 'aeroplane':
                        ann_dict[image_fname].append(label2id['airplane'])
                    elif cat == 'motorbike':
                        ann_dict[image_fname].append(label2id['motorcycle'])
                    else:
                        ann_dict[image_fname].append(label2id[cat])
    # create label matrix:
    ...
    np.save(os.path.join(args.save_path, 'mixed_formatted_' + phase + '_labels.npy'), label_matrix)
    np.save(os.path.join(args.save_path, 'mixed_formatted_' + phase + '_images.npy'), np.array(image_list[phase]))
