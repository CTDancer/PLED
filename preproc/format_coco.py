...
import argparse
import numpy as np
from consts import label_list, id2label, label2id, list_length
...
for split in ['train', 'val']:
    ...
    label_matrix = np.zeros((num_images, list_length))
    ...
    for i in range(len(D['annotations'])):
        ...
        category = meta['category_list'][category_index]
        if category in pascal_labels:
            label_matrix[row_index][category_index] = np.nan
            image_ids[row_index] = np.nan
        else:
            label_matrix[row_index][category_index] = 1
            image_ids[row_index] = int(image_id)
    
    nan_mask = ~np.isnan(label_matrix).any(axis=1)
    label_matrix = label_matrix[nan_mask]
    image_ids = image_ids[nan_mask]
    ...
    np.save(os.path.join(args.save_path, 'mixed_formatted_' + split + '_labels.npy'), label_matrix)
    np.save(os.path.join(args.save_path, 'mixed_formatted_' + split + '_images.npy'), image_ids)
